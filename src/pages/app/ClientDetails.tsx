...
export default function ClientDetails() {
  ...
  function handleDownloadPdf() {
    ...
    function renderSection(title: string, data: Record<string, any>): string {
      const entries = Object.entries(data);
      if (entries.length === 0) return "";
      return `<div class="section">
        <div class="section-title">${title}</div>
        ${entries.map(([k, v]) => `<div class="row"><span class="label">${k}</span><span class="value">${v}</span></div>`).join("")}
      </div>`;
    }

    printWindow.document.write(`<html><head><title>CA-flow — Calculations</title><style>
      body{font-family:system-ui,sans-serif;padding:40px;color:#222}
      h1{font-size:20px;margin-bottom:4px}
      .client{color:#666;margin-bottom:24px}
      .calc{margin-bottom:32px;padding-bottom:24px;border-bottom:1px solid #ddd}
      .calc-name{font-size:16px;font-weight:600}
      .calc-date{font-size:12px;color:#888;margin-bottom:4px}
      .section{margin-top:16px}
      .section-title{font-size:13px;font-weight:600;color:#444;margin-bottom:8px;text-transform:uppercase;letter-spacing:0.5px}
      ...
      .disclaimer{margin-top:40px;font-size:10px;color:#999;border-top:1px solid #eee;padding-top:12px}
    </style></head><body>
      <h1>CA-flow</h1>
      <div class="client">${client?.name ?? ""} ${client?.pan ? "| PAN: " + client.pan : ""} ${client?.gstin ? "| GSTIN: " + client.gstin : ""}</div>
      ${selectedCalcs.map((calc) => {
        ...
        return `<div class="calc">
          <div class="calc-name">${calc.title}</div>
          <div class="calc-date">${calc.subtitle ? calc.subtitle + " · " : ""}Saved: ${calc.savedAt || "—"}</div>
          ${hasData ? renderSection("Inputs", inputs) + renderSection("Results", outputs) : '<div class="no-data">No calculation data available</div>'}
        </div>`;
      }).join("")}
      <div class="disclaimer">This report was generated by CA-flow. Calculations follow Indian tax rules as per FY 2025-26. Always verify with original circulars before filing.</div>
    </body></html>`);
