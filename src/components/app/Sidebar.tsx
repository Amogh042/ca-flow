...
const NavItem = ({ to, label, icon: Icon, glow, collapsed }: { to: string; label: string; icon: ComponentType<SVGProps<SVGSVGElement>>; glow?: boolean; collapsed?: boolean; }) => (
  <NavLink
    to={to}
    className={({ isActive }) =>
      cn(
...
export const Sidebar = React.memo(function Sidebar() {
  ...
  return (
    <aside className={cn(
      "hidden md:flex flex-col shrink-0 h-screen sticky top-6 transition-[width]",
      "glass-sidebar",
      ...
      <div className="border-t p-3 space-y-2" style={{ borderColor: "var(--border-color)" }}>
        ...
        <div className={cn("flex items-center gap-3 px-3 py-2 rounded-lg transition-colors", collapsed && "justify-center") }>
          <div className={cn("h-8 w-8 rounded-full grid place-items-center text-xs font-bold text-white bg-gradient-orange")}>
            {userInitial}
          </div>
          {!collapsed && (
            user ? (
